...
void multiMatrix(size_t *mA, size_t *mB, size_t *mC, int D){
	size_t Suma, *pA, *pB;
	//Genera paralelismo generando hilos de trabajo via Openmp
	#pragma omp parallel for private(pA, pB, Suma)

	//Realiza la multiplicación de matrices segun el algortimo basico de filas por columnas
	for(int i=0; i<D; i++){
		for(int j=0; j<D; j++){
			*pA = mA+i*D;	
			*pB = mB+j; //pB = mB+(j*D);	
			Suma = 0;
			//Se utiliza algoritmo filas por columnas
			for(int k=0; k<D; k++, pA++, pB+=D){
				Suma += *pA * *pB;
			}
			//Almacena el valor resultado en donde coresponde
			mC[i*D+j] = Suma;
		}
	}
}
